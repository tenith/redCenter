...
export class RemoteSEPComponent implements OnInit, OnDestroy {
  ...
  constructor(private route: ActivatedRoute,private firebaseAuth: FirebaseAuthenticationService, private firestoreUserService: FirestoreUserService) { }
      ...
      .subscribe(params => {
        ...
        if(btoa(email) == token){
          ...
          this.firebaseAuth.AnynomousAuth(email).then(() => {
            this.firestoreUserService.getFirestoreUserFromServerByEmail(email).then((doc)=> {
              this.loading = false;
              if(doc.exists) {
