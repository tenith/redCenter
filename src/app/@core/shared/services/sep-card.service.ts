...
export class SepCardService {
  ...
  sepCardLocalDBName: string =
    localStorageCollection.sepLocalDBNameCollectionName;
  ...
  private getAllSepCardsFromTMC(): Observable<any> {
    const myParams = new HttpParams().set(
      "email",
    ...
    return this.httpClient.get<any>(this.redBookTMCURL, {
      params: myParams,
      headers: myHeaders,
    });
  }
  ...
  getAllSepCards(): Observable<any> {
    if (settings.redBookTMC) return this.getAllSepCardsFromTMC();
  ...
  */
  getBase64ImageFromUrl(imageUrl: string, name: string): void {
    ...
    this.httpClient
      .get<any>(imageUrl, { headers: headers, responseType: "blob" as "json" })
      .subscribe((data) => {
        ...
        reader.addEventListener("load", () => {
          ...
          if (
            ...
            this.dbService
              .add("certificates", { link: name, uri: reader.result })
              .subscribe();
        });
  ...
  saveAllCertificate(): void {
    ...
    for (let i = 0; i < this.oneSepCards.length; i++) {
      if (this.oneSepCards[i].Link != "") {
        ...
        const id =
          this.oneSepCards[i].Name.replace(/ /g, "_") +
          this.oneSepCards[i].Attended.replace(/ /g, "_");
        this.dbService
          .getByIndex("certificates", "link", id)
          .subscribe((data) => {
            if (data == null) {
