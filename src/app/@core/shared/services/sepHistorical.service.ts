...
import { localStorageCollection, sepCourseDisplayOptions, sepCourseOptions } from '../../../../environments/myconfigs';
...
export class SepHistoicalService {  
  ...
  sepHistoricalDBName: string = localStorageCollection.sepHistoricalCollectionName; 
  ...
  constructor(private firestoreUser: FirestoreUserService) { 
    ...
    if(temp != null) 
        this.fullHistory = JSON.parse(temp);
  }
  ...
  public getHistorical(courseName: string): OneSepCard[] {
    if(courseName in this.fullHistory)
        return this.fullHistory[courseName];
    else
        return [];
  }

  public save(): void {
    localStorage.setItem(this.sepHistoricalDBName, JSON.stringify(this.fullHistory));
  }
