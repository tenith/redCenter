...
export class TabService {
  ...
  loadUUIDTabsFromStorage(): void {
    ...
    if (temp != null) this.viewUUIDTabs = JSON.parse(temp);
  }
  ...
  saveCurrectStateTabs(): void {
    ...
    localStorage.setItem(
      localStorageCollection.tabCollectionName,
      JSON.stringify(this.viewUUIDTabs),
    );
  }
  ...
  removeViewTab(uuid: string): void {
    ...
    const index = this.viewUUIDTabs.indexOf(uuid);
    //FOUND VIEWTAB....
    if (index > -1) this.viewUUIDTabs.splice(index, 1);
