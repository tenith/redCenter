...
export class FirestoreUserService {
  ...
  firestoreUserDBName: string =
    localStorageCollection.firestoreUserDBNameCollectionName;
  ...
  public reviseAuthorization(): void {
    if (
      this.firestoreUser.level == userLevel.admin ||
      this.firestoreUser.level == userLevel.moderatore
    )
      this.canCreateFlag = true;

    if (this.firestoreUser.level == userLevel.admin) this.canDeleteFlag = true;

    if (this.firestoreUser.level == userLevel.moderatore)
      this.isModerator = true;

    if (this.firestoreUser.level == userLevel.admin) this.isAdmin = true;
  }
  ...
  public isAcknowledge(code: string): number {
    ...
    const tempIndex = acknowledgeList.findIndex((object) => {
      return object.uuid === encodeURIComponent(code);
    });
  ...
  public hasRole(): boolean {
    ...
    if (this.firestoreUser == null || this.firestoreUser == undefined)
      return false;
    if (this.firestoreUser.role == null) return false;
    if (this.firestoreUser.role != "") return true;
  ...
  setInitialUser(cId: string, aoc: string, role: string): Promise<any> {
    ...
    return this.collectionRef
      .doc(this.firebaseAuthen.getFirebaseUser().email)
      .update({
        cId: this.firestoreUser.cId,
        role: this.firestoreUser.role,
        aoc: this.firestoreUser.aoc,
      });
  }
  ...
  addToken(token: string): void {
    ...
    this.collectionRef
      .doc(this.firebaseAuthen.getFirebaseUser().email)
      .ref.update({
        tokenList: firestore.firestore.FieldValue.arrayUnion(token),
      })
      .then(() => {
    ...
    roleCollection
      ...
      .collection(this.firestoreUser.role)
      .doc(firestoreCollection.groupTokenDocumentName)
      .ref.update({
        tokenList: firestore.firestore.FieldValue.arrayUnion(token),
      })
      .then(() => {
  ...
  async deleteToken(): Promise<any> {
    await this.collectionRef
      .doc(this.firebaseAuthen.getFirebaseUser().email)
      .ref.update({
        tokenList: firestore.firestore.FieldValue.arrayRemove(this.token),
      })
      .then(() => {
