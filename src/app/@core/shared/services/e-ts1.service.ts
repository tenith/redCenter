...
export class ETS1Service {
  ...
  constructor(
    ...
    const isMySession = this.afs.collection(this.collectionName, (ref) =>
      ref.where("staffNo1", "==", this.firestoreUser.getFirestoreUser().cId),
    );
    const isMine = this.afs.collection(this.collectionName, (ref) =>
      ref.where(
        "ownerEmail",
        "==",
    ...
    const isSubmit = this.afs.collection(this.collectionName, (ref) =>
      ref.where("submitDateTime", "!=", ""),
    );

    let isMineObservable: Observable<ETS1Data[]> = isMine
      .snapshotChanges()
      .pipe(
        map((actions) =>
          actions.map((a) => {
    ...
    let isMySessionObservable: Observable<ETS1Data[]> = isMySession
      .snapshotChanges()
      .pipe(
        map((actions) =>
          actions.map((a) => {
    ...
    isMineObservable = combineLatest([
      isMineObservable,
      isMySessionObservable,
    ]).pipe(
      map(([data1, data2]) => {
    ...
    if (this.firestoreUser.isAdmin || this.firestoreUser.isModerator) {
      let isSubmitObservable: Observable<ETS1Data[]> = isSubmit
        .snapshotChanges()
        .pipe(
          map((actions) =>
            actions.map((a) => {
      ...
      const totalObservable = combineLatest([
        isMineObservable,
        isSubmitObservable,
      ]).pipe(
        map(([data1, data2]) => {
    ...
    this.collectionRef = this.afs.collection(this.collectionName, (ref) =>
      ref.where(
        "ownerEmail",
        "==",
        this.firestoreUser.getFirestoreUser().email,
  ...
  geteTS1FromCacheByUUID(uuid: string): ETS1Data {
    ...
    for (let i = 0; i < this.eTS1ListCache.length; i++) {
      if (this.eTS1ListCache[i].uuid == uuid) temp = this.eTS1ListCache[i];
    }
    return temp;
  }
