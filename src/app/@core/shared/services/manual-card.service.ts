...
export class ManualCardService {
  manualCardLocalDBName: string = localStorageCollection.manualLocalDBNameCollectionName;  
  manualCards: OneSepCard[];

  constructor(private fileUploadService: FileUploadDatabaseService ,public fireBaseAuthService: FirebaseAuthenticationService, public dbService: NgxIndexedDBService) { 
  ...
  saveAllCertificate(): void{
    if(this.manualCards == null)
      return;

    for(let i=0;i<this.manualCards.length;i++){     
      this.dbService.getByIndex("certificates",'link', this.manualCards[i].Link).subscribe(async (data)=> {
        if(data == null){
          await this.getImageFromFireDB(this.manualCards[i].Link).then(async downloadURL => {
            await this.getBase64ImageFromUrl(downloadURL)
              .then(result => {
                this.dbService.add("certificates",{link:this.manualCards[i].Link,uri:result}).subscribe();
              })
  ...
  saveAllCards(oneSepCards: OneSepCard[]): void {
    ...
    localStorage.setItem(this.manualCardLocalDBName,JSON.stringify(oneSepCards));
