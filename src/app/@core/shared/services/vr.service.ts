...
export class VrService {
  ...
  constructor(
    ...
    if (this.firestoreUser.isModerator || this.firestoreUser.isAdmin) {
      ...
      this.vrList = this.collectionRef.snapshotChanges().pipe(
        map((actions) =>
          actions.map((a) => {
      ...
      this.collectionRef = this.afs.collection(this.collectionName, (ref) =>
        ref.where(
          "crewsId",
          "array-contains",
          this.firestoreUser.getFirestoreUser().cId,
        ),
      );
      this.vrList = this.collectionRef.snapshotChanges().pipe(
        map((actions) =>
  ...
  public getMyVR(): Observable<VrDetail[]> {
    const temp = this.afs.collection(this.collectionName, (ref) =>
      ref.where(
        "crewsId",
        "array-contains",
        this.firestoreUser.getFirestoreUser().cId,
    ...
    return temp.snapshotChanges().pipe(
      map((actions) =>
        actions.map((a) => {
          const doc = a.payload.doc.data() as VrDetail;
          return { ...doc } as VrDetail;
