...
export class AnnouncementService {
  ...
  constructor(
    ...
    this.announcements = this.collectionRef.snapshotChanges().pipe(
      map((actions) =>
        actions.map((a) => {
  ...
  makeAnnouncement(id: string): Promise<any> {
    let params = new HttpParams()
      .set("announcementsID", id)
      .set("timeStamp", new Date().valueOf());
  ...
  addAnnouncement(announcement: Announcement): Promise<any> {
    return this.collectionRef
      .doc(encodeURIComponent(announcement.code))
      .set(announcement);
  }
  ...
  getAnnouncementFromCache(code: string): Announcement {
    if (!this.cacheAnnouncement) return null;

    const tempIndex = this.cacheAnnouncement.findIndex((object) => {
      return encodeURIComponent(object.code) === code;
    });
    if (tempIndex >= 0) return this.cacheAnnouncement[tempIndex];
    else return null;
  }
  ...
  addAcknowledgement(code: string, signature: Signature): Promise<any> {
    return this.collectionRef
      .doc(code)
      .ref.update({
        signatures: firestore.firestore.FieldValue.arrayUnion(signature),
      });
  }
