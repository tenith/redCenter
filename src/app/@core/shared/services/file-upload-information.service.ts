...
export class FileUploadInformationService {
  ...
  public async reloadData(): Promise<any> {
    ...
    ref.get().then((docSnapshot) => {
      if (!docSnapshot.exists) {
        this.collectionRef
          .doc(this.email)
          .set({ files: [], needVerify: false });
      } else {
        this.fileUploadInformations = [
          ...docSnapshot.data().files,
        ...
        else if (docSnapshot.data().needVerify != needVerify)
          this.setNeedVerify(this.email, needVerify);
      }
    });
  }
    ...
    ref.get().then((docSnapshot) => {
      if (docSnapshot.exists) {
        ...
        for (let i = 0; i < temp.length; i++) {
          if (temp[i].verify == undefined) continue;
  ...
  public getFileUploadInformation(email: string): Promise<any> {
    ...
    if (newRef == null || newRef == undefined) return null;
    return newRef.get();
  }
