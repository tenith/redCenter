...
export class PostingDocumentsAmendmentComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.effectiveDateSubscription = (
      this.postingForm.get("publishedDate") as FormControl
    ).valueChanges.subscribe((value) => this.validateEffectiveDate());
    this.endDateSubscription = (
      this.postingForm.get("effectiveDate") as FormControl
    ).valueChanges.subscribe((value) => this.validateEndDate());
    this.diffDateSubscription = (
      this.postingForm.get("endDate") as FormControl
    ).valueChanges.subscribe((value) => this.calDiffDate());
  }

  calDiffDate(): void {
    const effectiveDate = new Date(
      this.postingForm.get("effectiveDate").value,
    ).valueOf();
    const endDate = new Date(this.postingForm.get("endDate").value).valueOf();

    if (endDate >= effectiveDate)
      this.diffDate = (endDate - effectiveDate) / (1000 * 60 * 60 * 24);
    else this.diffDate = 0;
  }
  ...
  ngOnDestroy(): void {
    ...
    if (this.diffDateSubscription) this.diffDateSubscription.unsubscribe();
  }
  ...
  validateEndDate(): void {
    ...
    if (endDate < effectiveDate) this.postingForm.get("endDate").setValue("");
  }

  validateEffectiveDate(): void {
    const publishedDate = new Date(
      this.postingForm.get("publishedDate").value,
    ).valueOf();
    const effectiveDate = new Date(
      this.postingForm.get("effectiveDate").value,
    ).valueOf();
  ...
  edit(event: any): void {
    ...
    if (this.postingForm.value.body == (event as string)) return;
