...
export class DocumentComponent implements OnInit {
  ...
  settings = {
    ...
    columns: {
      ...
      dateTime: {
        title: "Date Time of Acknowledge",
        filter: false,
        sort: false,
    ...
    this.announcementService
      ...
      .then((doc) => {
        ...
        if (this.announcement == null) {
          ...
          this.toastr.danger(
            "Error",
            "Invalid Document, Please check your dashboard again.",
            { duration: 5000 },
          );
          return;
        }
  ...
  revisedAcknowledge(): void {
    this.acknowledgeRequired =
      this.announcement.acknowledge == "No" ? false : true;
    ...
    this.notificationService.deleteNotificationReadOnlyDocuemntByCode(
      this.announcement.code,
    );
  ...
  sign(code: string, title: string): void {
    ...
    const invoice = {
      uuid: encodeURIComponent(code),
      title: title,
      dateTime: timeStamp,
    } as Invoice;

    this.announcementService
      .addAcknowledgement(encodeURIComponent(code), tempSignature)
      .then(() => {
        ...
        this.firestoreUserService
          ...
          .then(() => {
            ...
            this.toastr.primary(
              "Completed",
              "Acknowledge has been save into your device",
            );

            this.notificationService.deleteNotificationDocuemntByCode(
              this.announcement.code,
            );
          })
          .catch((error) => {
            ...
            this.toastr.danger(
              "error",
              "Acknowledge has been failed to save into your device",
            );
          });
      })
      .catch((error) => {
