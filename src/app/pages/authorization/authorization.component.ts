...
export class AuthorizationComponent implements OnInit, OnDestroy {
  ...
  search(): void {
    ...
    this.firestoreUserService
      .getFirestoreUserByEmail(this.searchEmail)
      .then((data) => {
        this.loading = false;
        if (data != null) {
  ...
  delete(): void {
    if (this.firestoreUser == null) return;
    ...
    this.dialogRef.onClose.subscribe((confirmation) => {
      if (confirmation == "confirm") {
        this.firestoreUserService
          .deleteFirestoreUser(this.firestoreUser)
          .then(() => {
            this.toastr.primary(
              "Completed",
              "Delete " + this.firestoreUser.email + " completed",
              { duration: 5000 },
            );
            this.reset();
          })
  ...
  update(): void {
    this.firestoreUserService
      .reviseFirestoreUser(this.firestoreUser)
      .then(() => {
        this.toastr.primary(
          "Completed",
          "Updated " + this.firestoreUser.email + " completed",
          { duration: 5000 },
        );
        this.reset();
      })
