...
export class FileUploadComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  ngAfterViewInit(): void {
    this.uploadFormSubject = this.uploadForm.valueChanges.subscribe((value) => {
      this.reviseRequired();
    });
  }

  ngOnDestroy(): void {
    if (!this.uploadFormSubject) this.uploadFormSubject.unsubscribe();
  }

  refreshDocList(): void {
    if (this.email == undefined) this.availableDocList = [];
    else
      this.availableDocList = this.availableDocList.filter(
        (item) => !this.alreadyUpload.includes(item),
      );
  ...
  ngOnInit(): void {
    ...
    if (this.email != undefined)
      this.firestoreUserService
        .getFirestoreUserByEmail(this.email)
        .then((doc) => {
          this.availableDocList = sepMandatory[doc.data().role];
          this.dataReady++;

          if (this.dataReady >= 2) this.refreshDocList();
        });
    else {
    ...
    this.fileUploadInformationService
      .getFileUploadInformationSnapshotByEmail(tempEmail)
  ...
  async upload(): Promise<void> {
    ...
    if (this.email == undefined) {
      ...
      await this.fileUploadDatabaseService
        .uploadFileByAdmin(this.fileToUpload, this.uploadForm.value, this.email)
        .then((response) => {
          if (response) {
            this.toastr.primary("Completed", "Upload file completed", {
              duration: 5000,
