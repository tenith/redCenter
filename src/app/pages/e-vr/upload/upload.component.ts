  ...
  uploadFile(): void {
    ...
    fileReader.onload = (e) => {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (
          data.includes(
            '<td colspan="18" rowspan="2" class="s18">Version:&nbsp;18/01',
          )
        ) {
        ...
        if (data.includes("LOCAL&nbsp;DATE:")) {
          tempVR.date = this.changeDateFormat(
            this.extractValueByKey(
              data,
              '<td colspan="6" rowspan="3" class="s6">',
              "</td>",
            ),
          );
        }
        ...
        if (crewChecked && data.includes('<td colspan="3" class="s31">')) {
          const id = this.extractValueByKey(
            data,
            '<td colspan="6" class="s32">',
            "</td>",
          );
          data = this.reviseData(data, '<td colspan="6" class="s32">');
          if (id.length != 7) continue;

          const position = this.extractValueByKey(
            data,
            '<td colspan="6" class="s32">',
            "</td>",
          );
          const name = this.extractValueByKey(
            data,
            '<td colspan="43" class="s33">',
            "</td>",
          );
          const reportTime = this.revisedTime(
            this.extractValueByKey(
              data,
              '<td colspan="10" class="s32">',
              "</td>",
            ),
          );
        ...
        if (flightChecked && data.includes('<td colspan="2" class="s32">')) {
          const flightNumber = this.extractValueByKey(
            data,
            '<td colspan="2" class="s32">',
            "</td>",
          );
          data = this.reviseData(data, '<td colspan="2" class="s32">');
          if (flightNumber.includes(">") || flightNumber.length == 0) continue;
          ...
          const std = this.revisedTime(
            this.extractValueByKey(data, ">", "</td>"),
          );
          data = this.reviseData(data, ">");

          const sta = this.revisedTime(
            this.extractValueByKey(data, ">", "</td>"),
          );
          data = this.reviseData(data, ">");

          const blk = this.revisedTime(
            this.extractValueByKey(data, ">", "</td>"),
          );
          data = this.reviseData(data, ">");
