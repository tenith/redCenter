...
import {
  VrDetail,
  defaultVR,
} from "../../../@core/shared/interfaces/vr-detail";
...
export class WorkspaceComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  constructor(
    ...
    private vrCommentService: VRCommentGoogleSheetsSerivce,
  ...
  ngOnInit(): void {
    ...
    if (tempId != null) {
      ...
      const savedVR = localStorage.getItem(
        localStorageCollection.vrLocalDBNameCollectionName,
      );
      if (savedVR != null) {
  ...
  noCrewsFlights(): boolean {
    if (
      this.getFormArrayByName("crews").length == 0 ||
      this.getFormArrayByName("flights").length == 0
    ) {
      return true;
    }
  ...
  calMyDuty(): string {
    ...
    if (numberOfLanding == 0) return "00:00";

    const landingTime = this.getFormArrayByName("flights").at(
      numberOfLanding - 1,
    ).value.onChk;
  ...
  maxDuty(): string {
    ...
    if (numberOfLanding >= 8) numberOfLanding = 8;
  ...
  addComments(commentList: CommentDetail[]): void {
    ...
    for (let i = 0; i < this.totalComments; i++) {
      ...
      commentFormGroup.valueChanges.subscribe((value) => {
        this.reviseComments();
      });
  ...
  addCrew(crewDetail: CrewDetail): void {
    const newCrewForm = this.formBuilder.group({
      id: [
        crewDetail.id,
        [Validators.required, Validators.pattern("1[0-9]{6}$")],
      ],
  ...
  reviseCrew(): void {
    ...
    for (let i = 0; i < crewGroup.length; i++) {
      this.crewList.push({
        ...(crewGroup.at(i).value as unknown as CrewDetail),
      });
    }
  ...
  edit(event: any): void {
    if (this.form.value.comment == (event as string)) return;
  ...
  addFlight(flightDetail: FlightDetail): void {
    const newFlightForm = this.formBuilder.group({
      fltNO: [
        flightDetail.fltNO,
        [Validators.required, Validators.pattern("[0-9]{1,4}[a-zA-Z]?$")],
      ],
      registration: [
        flightDetail.registration,
        [
          Validators.required,
          Validators.pattern("^[hH]{1}[sS]{1}-[a-zA-Z]{3}$"),
        ],
      ],
      from: [
        flightDetail.from,
      ...
      blk: [
        this.subtractTime(flightDetail.sta, flightDetail.std),
        Validators.required,
      ],
      ...
      landing: [
        this.subtractTime(flightDetail.sta, "00:10"),
        Validators.required,
      ],
      ...
      departureDelayTime1: [
        flightDetail.departureDelayTime1 == ""
          ? "00:00"
          : flightDetail.departureDelayTime1,
      ],
      departureDelayCode1: [
        flightDetail.departureDelayCode1 == ""
          ? "-"
          : flightDetail.departureDelayCode1,
      ],

      departureDelayTime2: [
        flightDetail.departureDelayTime2 == ""
          ? "00:00"
          : flightDetail.departureDelayTime2,
      ],
      departureDelayCode2: [
        flightDetail.departureDelayCode2 == ""
          ? "-"
          : flightDetail.departureDelayCode2,
      ],

      departureDelayTime3: [
        flightDetail.departureDelayTime3 == ""
          ? "00:00"
          : flightDetail.departureDelayTime3,
      ],
      departureDelayCode3: [
        flightDetail.departureDelayCode3 == ""
          ? "-"
          : flightDetail.departureDelayCode3,
      ],

      arrivalDelayTime1: [
        flightDetail.arrivalDelayTime1 == ""
          ? "00:00"
          : flightDetail.arrivalDelayTime1,
      ],
      arrivalDelayCode1: [
        flightDetail.arrivalDelayCode1 == ""
          ? "-"
          : flightDetail.arrivalDelayCode1,
      ],
    });
  ...
  reCalcBlk(): void {
    ...
    for (let i = 0; i < flightGroup.length; i++) {
      ...
      flightGroup
        .at(i)
        .get("blk")
        .setValue(this.subtractTime(temp.value.sta, temp.value.std), {
          emitEvent: false,
        });
      flightGroup
        .at(i)
        .get("actBlk")
        .setValue(this.subtractTime(temp.value.onChk, temp.value.offChk), {
          emitEvent: false,
        });

      flightGroup
        .at(i)
        .get("arrivalDelayTime1")
        .setValue(this.subtractDelay(temp.value.onChk, temp.value.sta), {
          emitEvent: false,
        });

      let totalDepartureDelayTime = this.subtractDelay(
        temp.value.offChk,
        temp.value.std,
      );
      ...
      for (let j = 0; j < 3; j++) {
        if (totalDepartureDelayTime == "00:00") {
          flightGroup
            .at(i)
            .get("departureDelayTime" + String(j + 1))
            .setValue("00:00", { emitEvent: false });
        }

        if (
          this.isGreater(
            flightGroup.at(i).get("departureDelayTime" + String(j + 1)).value,
            totalDepartureDelayTime,
          )
        ) {
          flightGroup
            .at(i)
            .get("departureDelayTime" + String(j + 1))
            .setValue(totalDepartureDelayTime, { emitEvent: false });
        ...
        if (
          ...
          totalDepartureDelayTime = this.subtractDelay(
            totalDepartureDelayTime,
            flightGroup.at(i).get("departureDelayTime" + String(j + 1)).value,
          );
        }
      ...
      for (let j = 0; j < 3; j++) {
        if (
          ...
          flightGroup
            .at(i)
            .get("departureDelayCode" + String(j + 1))
            .clearValidators();
          flightGroup
            .at(i)
            .get("departureDelayCode" + String(j + 1))
          ...
          if (
            flightGroup.at(i).get("departureDelayCode" + String(j + 1)).value ==
            "-"
          )
            flightGroup
              .at(i)
    ...
    this.dialogRef = this.dialogService.open(DeleteConfirmationComponent, {
      context: {
        data: {
          ...
          information:
            "You try to delete crew #" + (index + 1) + " from this VR.",
        },
      },
    });

    this.dialogRef.onClose.subscribe((confirm) => {
      if (confirm == "affirm") this.removeCrew(index);
    });
  }
    ...
    this.dialogRef = this.dialogService.open(DeleteConfirmationComponent, {
      context: {
        data: {
          title: "Do you want to delete information?",
          information:
            "You try to delete flight #" + (index + 1) + " from this VR.",
        },
      },
    });

    this.dialogRef.onClose.subscribe((confirm) => {
      if (confirm == "affirm") this.removeFlight(index);
    });
  }
  ...
  private addTime(a: string, b: string): string {
    ...
    if (resulttotal >= 1440) resulttotal -= 1440;
  ...
  private subtractTime(a: string, b: string): string {
    ...
    let resulttotal = atotal - btotal;
    if (resulttotal < 0) resulttotal += 1440;
  ...
  softReset(): void {
    ...
    this.saveFormSub = this.form.valueChanges
    ...
    this.dialogRef = this.dialogService.open(DeleteConfirmationComponent, {
      context: {
        data: {
          ...
          information:
            "You try to reset this VR, All your VR information will be deleted",
        },
      },
    });
  ...
  prepareData(): VrDetail {
    ...
    let idList: string[] = [];
    for (let i = 0; i < crews.length; i++) idList.push(crews[i].id);
  ...
  submit(): void {
    ...
    this.vrService
      .saveVRwithSubmitTime(vr, formattedDate)
      .then(() => {
        this.submitStatus = true;
        this.toastr.primary("Completed", "VR has been submitted", {
          duration: 10000,
        });
        //NOTIFICAtion to training....
        this.sendToGoogleSheet(vr.date, vr.comments);
        this.sendNotificationToOPS(
          uuid,
  ...
  sendToGoogleSheet(date: string, comments: CommentDetail[]): void {
    this.vrCommentService.post(date, comments).subscribe((response) => {
      if (
        response
          .toString()
          .includes("VR Comments data have been uploaded to OPS DATA studio.")
      ) {
        this.toastr.primary(
          "Completed",
          "Upload additional comments to Server completed",
          { duration: 10000 },
        );
      }
    });
