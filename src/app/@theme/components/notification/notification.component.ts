...
export class NotificationComponent implements OnInit {
  ...
  constructor(
    private dialogRef: NbDialogRef<NotificationComponent>,
  ...
  markAsRead(notification: Notification): void {
    ...
    if (notification.type == "PERSONAL_DOCUMENT")
      this.router.navigate([notification.link], {
        queryParams: { email: notification.uuid },
      });

    if (notification.type == "VR")
      this.router.navigate([notification.link], {
        queryParams: { id: notification.uuid },
      });

    if (notification.type == "document")
      this.router.navigate([notification.link], {
        queryParams: { code: encodeURIComponent(notification.code) },
      });
