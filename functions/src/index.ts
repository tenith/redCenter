...
export const onDocumentUpdated = functions.firestore
  .document("/fileUploadInformation/{emailAddress}")
  .onUpdate((change, context) => {
    const newData = change.after.data();

    // console.log("onDocumentUpdated:  " + JSON.stringify(newData));

    if (newData.needVerify == true) return null;
    const docs = newData.files;
    for (let i = 0; i < docs.length; i++) {
      if (
        docs[i].description.includes("Cabin_Crew") &&
        docs[i].description.includes("Medical") &&
        docs[i].verify == false
      ) {
        const firestore = admin.firestore();
        const docRef = firestore
          .collection("/fileUploadInformation")
          .doc(context.params.emailAddress);

        console.log("onDocumentUpdated: Set needVerify to be true");
        docRef.update({ needVerify: true });
        break;
      }
    }
...
export const sendCCDPersonalDocNotification = functions.https.onRequest(
  async (req, res) => {
    corsHandler(req, res, async () => {
      ...
      const payload: admin.messaging.MessagingPayload = {
        ...
        notification: {
          ...
          click_action:
            "https://lightredcenter.web.app/pages/personal_documents",
...
export const sendPersonalDocNotification = functions.https.onRequest(
  async (req, res) => {
    corsHandler(req, res, async () => {
      //Your code here
...
export const sendETS1Notification = functions.https.onRequest(
  async (req, res) => {
    corsHandler(req, res, async () => {
      //Your code here
...
export const sendVRNotification = functions.https.onRequest(
  async (req, res) => {
    corsHandler(req, res, async () => {
      //Your code here
  ...
  corsHandler(req, res, async () => {
    ...
    await getAnnouncement(announcementsID)
      .then((doc) => {
        ...
        } else announcementsSnapshot = null;
      })
      .catch((error) => {
    ...
    const payload: admin.messaging.MessagingPayload = {
      ...
      notification: {
        ...
        icon: "https://cdn-icons-png.flaticon.com/512/9746/9746449.png",
        click_action:
          "https://lightredcenter.web.app/pages/documents_amendment/dashboard",
...
async function getGroupTokenList(group: string): Promise<any> {
  ...
  const db = admin.firestore();
  const tokenDocumentRef = db
    .collection("/groupTokenList")
    .doc("FD")
    .collection(group)
    .doc("tokenDocument");
